...
spam_logistic_mod <- function(split){
  model <- glm(spam ~ word_freq_your + word_freq_000 + word_freq_receive + word_freq_business + capital_run_length_longest, data = analysis(split), family = "binomial")
  test = assessment(split)
...
```
```{r}
library(modelr)  # for analysis and assessment
library(yardstick)  # for metric calculation
library(purrr)  # for map functions
library(dplyr)  # for data manipulation

# Update the function to properly handle metric calculation
spam_logistic_mod <- function(split) {
  model <- glm(spam ~ word_freq_your + word_freq_000 + word_freq_receive + 
               word_freq_business + capital_run_length_longest, 
  ...
  # Ensure predictions and observations are factors with correct levels
  my_obs <- factor(test$spam, levels = c("0", "1"))
  my_preds <- factor(my_preds, levels = c("0", "1"))

  # Prepare data frame for conf_mat()
  results <- data.frame(truth = my_obs, estimate = my_preds)

  # Calculate metrics directly ensuring only single values are used
  ...
  # Create a named vector of metrics
